...
function bfox_blog_init() {
	...
	if (!bfox_blog_option('disable-tooltips')) {
		wp_register_script('bfox-qtip', BFOX_URL . '/includes/js/jquery-qtip/jquery.qtip-1.0.0-rc3-custom.js', array('jquery'), BFOX_VERSION);
...
function bfox_ref_blog_url($ref_str) {
	...
	//if (empty($taglink))
		$taglink = get_option('home') . '/?s=' . $ref_str. '&jet_ajax_search_settings={"search_source"%3A["session"%2C"collection"]%2C"results_order_by"%3A"ID"%2C"results_order"%3A"desc"}&post_type=session%2Ccollection';
//	else {
...
function bfox_blog_option($key) {
	...
	if(!isset($options['disable-tooltips'])){
		$options['disable-tooltips'] = get_option('disable-tooltips');
	}
	return $options[$key];
}